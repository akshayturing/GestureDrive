...
def gesture_processor():
    ...
    while True:
        try:
            ...
            if current_time - last_update_time > cooldown:
                # Process motion gestures for navigation
                if hasattr(cam, 'current_motion') and cam.current_motion == "swipe_up":
                    file_controller.select_previous()
                    last_update_time = current_time
                elif hasattr(cam, 'current_motion') and cam.current_motion == "swipe_down":
                    file_controller.select_next()
                    last_update_time = current_time
                elif hasattr(cam, 'current_motion') and cam.current_motion == "swipe_left":
                    file_controller.navigate_directory("up")
                    last_update_time = current_time
                elif hasattr(cam, 'current_motion') and cam.current_motion == "swipe_right":
                    file_controller.navigate_directory("down")
                    last_update_time = current_time
                    
                # Process selection gestures
                if hasattr(cam, 'current_selection_gesture') and cam.current_selection_gesture:
                    file_controller.process_selection_gesture(cam.current_selection_gesture)
                    last_update_time = current_time
            
            time.sleep(0.1)  # Sleep to avoid excessive CPU usage
        except Exception as e:
            print(f"Error in gesture processor: {e}")
...
def api_gestures():
    ...
    cam = get_camera()
    gesture_data = {
        'gesture': getattr(cam, 'current_gesture', None),
        'motion': getattr(cam, 'current_motion', None),
        'selection_gesture': getattr(cam, 'current_selection_gesture', None)
    }
    return jsonify(gesture_data)
...
def api_select():
    ...
    if index is not None:
        try:
            ...
            # Default to next/previous if select_item isn't implemented
            success = False
    elif action == 'next':
        success = file_controller.select_next()
    elif action == 'previous':
