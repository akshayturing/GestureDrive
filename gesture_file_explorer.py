...
class GestureFileController:
    def __init__(self, camera, file_manager):
        self.camera = camera
        self.file_manager = file_manager
        
        # Initialize the gesture configuration manager
        self.config_manager = GestureConfigManager("config/gesture_config.json")
        
        # Set the initial context
        self.config_manager.set_context("fileBrowser")

    def process_frame(self):
        """
        Process the current frame from the camera and handle any detected gestures.
        """
        # Get the current gesture and motion from the camera
        gesture = self.camera.current_gesture
        motion = self.camera.current_motion
        
        # Determine the combined gesture (e.g., 'point_up + swipe_right')
        combined_gesture = gesture
        if motion and motion != "stationary":
            combined_gesture = motion
        
        # Skip if no gesture detected
        if not combined_gesture or combined_gesture == "unknown":
            return
        
        # Process the detected gesture and get the corresponding action
        action_details = self.config_manager.process_gesture(combined_gesture)
        
        if action_details:
            # Execute the action with the current file manager as context
            context_data = {
                "file_manager": self.file_manager,
                "current_path": self.file_manager.current_dir,
                "selected_item": self.file_manager.selected_item
            }
            
            result = self.config_manager.execute_action(action_details, context_data)
            
            # Log the executed action
            action_name = action_details.get("action", "unknown")
            logger.info(f"Executed action '{action_name}' for gesture '{combined_gesture}'")
    ...
    def change_context(self, new_context):
        """
        Change the current gesture context (e.g., when switching between file browser and image viewer).
        """
        self.config_manager.set_context(new_context)
        
    def reload_configuration(self):
        """
        Force reload of the gesture configuration file.
        """
